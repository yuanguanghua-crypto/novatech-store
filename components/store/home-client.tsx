    ...
    <>
        ...
        <div className="container relative py-20 lg:py-28">
          <div className="max-w-2xl">
            ...
            <div className="flex flex-wrap gap-4">
              <Link
                href="/products"
                className="inline-flex items-center gap-2 bg-white text-brand-900 px-6 py-3 rounded-full
              ...
              <Link
                href="/quote"
                className="inline-flex items-center gap-2 border-2 border-white text-white px-6 py-3 rounded-full
                           font-semibold hover:bg-white hover:text-brand-900 transition-colors"
              >
                {t.hero_cta_quote}
              </Link>
            </div>
      ...
      {categories.length > 0 && (
        <section className="py-12">
          <div className="container">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">{t.home_categories_title}</h2>
              ...
              {categories.map((cat) => (
                <Link
                  ...
                  className="group flex flex-col items-center p-4 bg-white rounded-xl border border-gray-200
                             hover:border-brand-300 hover:shadow-md transition-all text-center"
                >
                  <span className="text-3xl mb-2">{cat.icon}</span>
                  <span className="text-sm font-medium text-gray-800 group-hover:text-brand-700 leading-tight">
                    {cat.name}
                  </span>
        ...
        <section className="py-12 bg-gray-50">
          <div className="container">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">{t.home_featured_title}</h2>
                <p className="text-gray-500 text-sm mt-1">{t.home_featured_subtitle}</p>
              </div>
              <Link
                href="/products"
                className="hidden md:flex items-center gap-1 text-sm text-brand-700 hover:text-brand-800 font-medium"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {featuredProducts.map((product) => (
                ...
                >
                  ...
                  <div className="p-3">
                    {product.brand && (
                      <div className="text-xs text-brand-600 font-medium mb-1">{product.brand.name}</div>
                    )}
                    <h3 className="text-sm font-medium text-gray-900 line-clamp-2 group-hover:text-brand-700">
                      {product.name}
                    </h3>
                    <div className="text-xs text-gray-400 mt-1">{t.products_sku}: {product.sku}</div>
      ...
      <section className="py-16 bg-gradient-to-r from-accent-600 to-accent-500 text-white">
        <div className="container text-center">
          ...
          <p className="text-accent-100 mb-8 max-w-xl mx-auto">{t.home_cta_subtitle}</p>
          <Link
            href="/quote"
            className="inline-flex items-center gap-2 bg-gray-900 text-white px-8 py-3 rounded-full
